...
def add_git_submodule(repo_name, repo_url, repo_version):
    subprocess.call(['git', 'submodule', 'add', '-f', '-b', repo_version, repo_url, repo_name])
...
repos_files = glob.glob('**/resources.repos', recursive=True)
# Process each .repos file
for repos_file in repos_files:
    parse_repos_file(repos_file, "resources")